 ...
 */
var _menuDetails = null;
var Canvas = /** @class */ (function () {
    function Canvas(laneBean) {
        this.vehicleListLeftCanvas = [];
        this.vehicleListRightCanvas = [];
        ...
        if (leftLane.checked) {
            ...
            this.canvasLeft = document.getElementById(this.canvasIdLeft);
            ...
            this.canvasLeft.addEventListener("contextmenu", function (evt) {
                ...
                if (Temple.vehiclePositionList != null && Temple.vehiclePositionList.length > 0) {
                    for (var i in Temple.vehiclePositionList) {
                        if (Temple.vehiclePositionList[i].CanvasId != this.id)
                            continue;
                        // $('#' + Temple.vehiclePositionList[i].CanvasId).unbind("contextmenu");
                        // $('#' + Temple.vehiclePositionList[i].CanvasId).bind('contextmenu',);
                        hit = false;
                        mousePos = MouseEventList.getMousePos(this, evt);
                        if ((Temple.vehiclePositionList[i].Lattitude < mousePos.x) && (mousePos.x < Temple.vehiclePositionList[i].LattitudeNew)
                            && (Temple.vehiclePositionList[i].Longitude < mousePos.y) && (mousePos.y < Temple.vehiclePositionList[i].LongitudeNew)) {
        ...
        if (rightLane.checked) {
            ...
            this.canvasRight = document.getElementById(this.canvasIdRight);
    ...
    Canvas.prototype.refreshCanvas = function (laneBean, canvasPosition) {
    };
    Canvas.prototype.drawImagesOnCanvas = function () {
        ...
        var leftLaneChecked = document.getElementById("leftLaneCheckBox");
        var rightLaneChecked = document.getElementById("rightLaneCheckBox");
    ...
    Canvas.prototype.drawLines = function (ctx, noOfPoints, moveToX, lineToX, lineToXHor, canvasHeight) {
        // if(ctx != null && ctx.)
        var val = Misc.getVerticalDrawLineDestance(noOfPoints, canvasHeight);
    ...
    Canvas.getImageXPointOnCanvas = function (canvasWidth, percentageLegComleted) {
    ...
    Canvas.getImageYPointOnCanvas = function (noOfPoints, noOfSourcePoints, routePosition) {
        var yPoint = 20;
    ...
    Canvas.clearCanvas = function (canvasLeft) {
        var canvasa = document.getElementById(canvasLeft);
